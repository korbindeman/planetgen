...
Work is done in a **project**. There are two shipped: Thalos (the default, the world being discovered) and Earth (the present-day reference fixture, `--earth` / `--project=earth`, no tree). A **variant** is a saved snapshot — layout seed, shape seed, body, genes, pins, ranges, and the stage data on the globe — not a saved seed. Every Save writes a new snapshot; the name decides which line it joins. Artifacts belong to a variant: `preview/<name>/v/<id>/`. Layout and Shape caches stamp a **schema** (`src/layout-artifact.js`, `src/shape-artifact.js`). Bump it when the file contract changes — a required field gone or renamed, or the packing. Do not bump it when the sim or the sketch pass changes. An added optional field still loads. Layout and Shape caches stamp a **schema** (`src/layout-artifact.js`, `src/shape-artifact.js`). Bump it when the file contract changes — a required field gone or renamed, or the packing. Do not bump it when the sim or the sketch pass changes. An added optional field still loads.
