...
Install the dependencies with =bun install=, then start the development server:

#+begin_src sh
bun run dev
#+end_src

Open =http://localhost:3000=. The browser reloads when the HTML or JavaScript
changes.

To create the production bundle used by =embed.html=, run =bun run build=.
